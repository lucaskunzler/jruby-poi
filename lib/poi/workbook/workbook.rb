...
module POI
  class Workbook < Facade(:poi_workbook, org.apache.poi.ss.usermodel.Workbook)
    ...
    HORIZONTAL_ALIGNMENT_CONSTANTS = Hash[*HORIZONTAL_ALIGNMENT.constants.map{|e| [e.downcase.to_sym, HORIZONTAL_ALIGNMENT.const_get(e)]}.flatten]

    VERTICAL_ALIGNMENT = org.apache.poi.ss.usermodel.VerticalAlignment
    VERTICAL_ALIGNMENT_CONSTANTS = Hash[*VERTICAL_ALIGNMENT.constants.map{|e| [e.downcase.to_sym, VERTICAL_ALIGNMENT.const_get(e)]}.flatten]

    FILL_PATTERN_TYPE = org.apache.poi.ss.usermodel.FillPatternType
    FILL_PATTERN_TYPE_CONSTANTS = Hash[*FILL_PATTERN_TYPE.constants.map{|e| [e.downcase.to_sym, FILL_PATTERN_TYPE.const_get(e)]}.flatten]
    ...
    def create_style options={}
      ...
      sym = :alignment
      set_value style, sym, options, HORIZONTAL_ALIGNMENT_CONSTANTS do | value |
        value
      end
      
      sym = :vertical_alignment
      set_value style, sym, options, VERTICAL_ALIGNMENT_CONSTANTS do | value |
        value
      end

      sym = :fill_pattern
      set_value style, sym, options, FILL_PATTERN_TYPE_CONSTANTS do | value |
        value
      end


      [:border_right, :border_left, :border_top, :border_bottom].each do | sym |
        set_value style, sym, options, BORDER_STYLE_CONSTANTS do | value |
          value
        end
      end
